...
class NpcControl extends Laya.Script3D {
    constructor() {
        super();
    }
    onEnable() {
        ...
        for (var i = 0; i < this.owner.numChildren; i++) {
            ...
            this.owner.getChildAt(i).addComponent(jiao_1.default);
            ;
        }
        //NPC闲游状态
        this.xianYouFun();
    ...
    jinGongJvDianFun() {
        //据点
        this.jvDian = GameManager_1.default.curScene.getChildByName("jvDian");
        this.jinGongSuDu = 1;
        this.state = "jinGongJvDian";
    }
    jinGongWanJiaFun() {
        //玩家
        this.wanJia = GameManager_1.default.curPlayerControl.owner;
        this.jinGongSuDu = 2;
        //作战先锋 模式速度处理
        if (GameManager_1.default.curLevelManager.moShi == "zuoZhanXianFeng") {
            ...
            if (huoDeJiChuSuDu.name == "jiangshi01") {
                this.jinGongSuDu = parseInt(GameManager_1.default.jsonShuJv["puTongJiangShiShuJv"]["yiDongSuDu"]);
            }
            else if (huoDeJiChuSuDu.name == "bianFu1") {
                this.jinGongSuDu = parseInt(GameManager_1.default.jsonShuJv["puTongBianFuShuJv"]["yiDongSuDu"]);
            }
            else if (huoDeJiChuSuDu.name == "bianFuBoss") {
                this.jinGongSuDu = parseInt(GameManager_1.default.jsonShuJv["puTongBianFuShuJv"]["yiDongSuDu"]) * 2;
            }
        }
        //据点占领模式要有一个行走的动画
        if (GameManager_1.default.curLevelManager.moShi == "jvDianZhanLing2") {
            if (this.owner.getChildAt(0).getComponent(jiao_1.default).state == "gouDaiTou" || this.owner.getChildAt(0).getComponent(jiao_1.default).state == "gouDai") {
            }
            else {
                var huoDeJiChuSuDu = this.owner.getChildAt(0).getChildByName("wuTi").getChildAt(0);
    ...
    xianYouFun() {
        ...
        this.xianYouSuDu = 1;
        this.suiJiFangXiang = GameManager_1.default.RandomInt(1, 4);
        this.state = "xianYou";
    }
    //巡逻
    xunLuoFun(dian1, dian2) {
        ...
        //告诉下面的板子巡逻
        this.owner.getChildAt(0).getComponent(jiao_1.default).xunLuoFun();
        this.state = "xunLuo";
    }
    //巡逻待机
    xunLuoDaiJiFun() {
        ...
        //告诉下面的板子待机
        for (var i = 0; i < this.owner.numChildren; i++) {
            this.owner.getChildAt(i).getComponent(jiao_1.default).daiJiFang();
        }
        this.owner.getComponent(Laya.Rigidbody3D).linearVelocity = new Laya.Vector3(0, 0, 0);
    ...
    AutoMove() {
        ...
        //进攻玩家
            ...
            //玩家正在激活时间风暴 就不移动
            if (GameManager_1.default.curPlayerControl.shiJianFengBaoKaiGuan == true || GameManager_1.default.curPlayerControl.bingFengKaiGuan == true || GameManager_1.default.curPlayerControl.state == "shengLi") {
                //单个身上的特效
            ...
            //据点占领 始终朝向玩家
            if (GameManager_1.default.curLevelManager.moShi == "jvDianZhanLing2") {
                var wt = this.owner.getChildAt(0).getComponent(jiao_1.default).wt;
                wt.transform.lookAt(GameManager_1.default.curPlayerControl.owner.transform.position, new Laya.Vector3(0, 1, 0), false);
        ...
        //巡逻
            ...
            var targetRota = null;
            var jvLi = 10000;
            if (GameManager_1.default.curLevelManager.moShi == "jvDianZhanLing" && this.owner.numChildren == 1) {
                targetRota = GameManager_1.default.curScene.getChildByName("jvDian").transform.position;
                jvLi = Laya.Vector3.distance(this.owner.transform.position, GameManager_1.default.curScene.getChildByName("jvDian").transform.position);
            }
            else {
                targetRota = GameManager_1.default.curPlayerControl.owner.transform.position;
                jvLi = Laya.Vector3.distance(this.owner.transform.position, GameManager_1.default.curPlayerControl.owner.transform.position);
            }
            ...
            if (jvLi <= 0.3) {
                ...
                if (GameManager_1.default.curLevelManager.moShi == "jvDianZhanLing" && this.owner.numChildren == 1) {
                    //随机生成炮台
                    var jvDianObj = GameManager_1.default.curScene.getChildByName("jvDian");
                    for (var i = 0; i < jvDianObj.getChildByName("banZiJiHe").numChildren; i++) {
                        if (jvDianObj.getChildByName("banZiJiHe").getChildAt(i).getChildByName("wuTi").numChildren == 0) {
                            //失败了就不生成
                            if (GameManager_1.default.curPlayerControl.state == "shiBai") {
                            }
                            else {
                                var paoTai = Laya.Sprite3D.instantiate(GameManager_1.default.models.getChildByName("jiQiang_paoTai"), jvDianObj.getChildByName("banZiJiHe").getChildAt(i).getChildByName("wuTi"));
                    ...
                    //玩家生成新的地块儿
                    for (var i = 0; i < this.owner.numChildren; i++) {
                        GameManager_1.default.curPlayerControl.addbanZiFun(0);
                    }
                }
                //海岛模式处理  组敌人狗带完 是胜利还是开门
                if (GameManager_1.default.curLevelManager.moShi == "haiDaoZhengBa") {
                    //当前关卡敌人地块儿<=0
                    if (GameManager_1.default.curLevelManager.dikuaiErZongShu <= 0) {
                        //当前关卡地块儿数量为0
                        if (GameManager_1.default.curLevelManager.banZiLeiXingArray.length - 1 == GameManager_1.default.curLevelManager.dangQianGuanQia) {
                            //玩家胜利触发
                            GameManager_1.default.curPlayerControl.shengLiChuLiFun();
                        }
                        else {
                            //玩家执行开门
                            GameManager_1.default.curPlayerControl.kaiMenFun();
                        }
                    }
                    else {
                    }
            ...
            this.owner.transform.position = targetRotaLerp;
            this.owner.transform.rotation = GameManager_1.default.curScene.getChildByName("WalkMobs2").transform.rotation;
        }
