...
class GxGameUtil extends cc_1.Component {
    constructor() {
        super(...arguments);
        this._shamingzi = ""; //标签名
        this._retryTime = 0;
    ...
    _fetchLw_zList() {
        ...
        let self = this;
        let item = cc_1.sys.localStorage.getItem("labelConfig2");
            ...
            self._httpGets(this._baseUrl + "/config2.json", false, function (res) {
                ...
                if (res) {
                    ...
                    if (parse.code == 1000) {
                        ...
                        cc_1.sys.localStorage.setItem("labelConfig2", JSON.stringify(self.lw_zList));
                        self._initIsS();
                    }
                    else {
                        //  if (self._retryTime < 15) {
    ...
    _httpGets(url, needHeader, callback) {
        ...
        if (window["ks"]) {
            ...
            let xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function () {
                (0, cc_1.log)(" label lw_z XML_HTTP_REQUEST onreadystatechange ");
                if (xhr.readyState === 4) {
        ...
        try {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && (xhr.status >= 200 && xhr.status < 400)) {
                    ...
                    if (window["cc"]) {
                        localStorage = cc_1.sys.localStorage;
                    }
                    else if (window["Laya"]) {
                        //@ts-ignore
