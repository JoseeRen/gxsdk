    ...
    var native_banner = _classThis = class extends _classSuper {
        ...
        show(native_data, on_show, on_hide) {
            ...
            if (this.node && (0, cc_1.isValid)(this.node)) {
                ...
                cc_1.game.addPersistRootNode(this.node);
                this.node.siblingIndex = 32765;
                this.node.active = false;
            }
            if ((0, cc_1.isValid)(this.node) && !this.node.active) {
                if (cc_1.view.getVisibleSize().width <= cc_1.view.getVisibleSize().height) {
                    this.node.scaleX = cc_1.view.getVisibleSize().width / this.node.width;
                    this.node.scaleY = cc_1.view.getVisibleSize().width / this.node.width;
                }
                else {
                    this.node.setScale((0, cc_1.v3)(0.7, 0.7));
                }
                if (GxAdParams_1.AdParams.oppo.bannerOnTop) {
                    let component = this.node.getComponent(cc_1.Widget);
                    if (component) {
        ...
        start() {
            ...
            this.node.on(cc_1.Node.EventType.TOUCH_END, this.on_click_adv, this);
        }
        update_view() {
            if (!(0, cc_1.isValid)(this.node) || !this.node.activeInHierarchy) {
                this.hide();
                return;
            }
        ...
        set_default_pos() {
            this.node.x = 0;
            this.node.y = -cc_1.view.getVisibleSize().height / 2;
        }
        onDisable() {
