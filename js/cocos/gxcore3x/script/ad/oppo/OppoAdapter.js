...
class OppoAdapter extends BaseAdapter_1.default {
   ...
   */
    ...
    showVideo(complete, flag = "") {
        // 过滤多次触发
        console.log("oppo showvideo::: ", this.get_time(), this.showVideoTime, this.videoShowing);
  ...
  */
    ...
    showAddDesktop(on_close, on_succ) {
        if (this.addIconNode && this.addIconNode !== undefined && (0, cc_1.isValid)(this.addIconNode.node))
            return;
            ...
            success: function (res) {
                ...
                if (res.data.token) {
                    ...
                    let request = () => {
                        self.requestNet({
                            //@ts-ignore
                            url: `${GxConstant.Code2SessionUrl}/ov?packageName=${GxAdParams_1.AdParams.oppo.packageName}&token=${res.data.token}&platform=oppo`,
                            successCallback: (res) => {
                                try {
