...
export default class PlayerControl extends Laya.Script3D {
    ...
    onStart() {
        ...
        //手指点击屏幕
        UIManager.gameUI.bj.on(Laya.Event.MOUSE_DOWN, this, this.mouseDown);
        //手指抬起屏幕
        Laya.stage.on(Laya.Event.MOUSE_UP, this, this.mouseUp);
        //手指移出
        Laya.stage.on(Laya.Event.MOUSE_OUT, this, this.mouseUp);

        this.xiangJiFa = GameManager.curScene.getChildByName("Camera");

        this.banZiJiHe = this.owner.getChildByName("banZiJiHe");//板子集合

        //每一个板子都绑定代码
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            this.banZiJiHe.getChildAt(i).addComponent(CwControl);
        }
        ...
        if (GameManager.curLevelManager.moShi == "haiDaoZhengBa") {
            GameManager.curPlayerControl.addbanZiFun(1);//激活一个初始板子
            GameManager.curPlayerControl.addbanZiFun(0);//激活一个初始板子
            GameManager.curPlayerControl.addbanZiFun(0);//激活一个初始板子
            GameManager.curPlayerControl.addbanZiFun(0);//激活一个初始板子

        } else if (GameManager.curLevelManager.moShi == "jvDianZhanLing") {
            GameManager.curPlayerControl.addbanZiFun(1);//激活一个初始板子
        } else if (GameManager.curLevelManager.moShi == "jvDianZhanLing2") {
            GameManager.curPlayerControl.addbanZiFun(1);//激活一个初始板子
        } else if (GameManager.curLevelManager.moShi == "xiaoDaoZuiHou") {
            GameManager.curPlayerControl.addbanZiFun(1);//激活一个初始板子
            GameManager.curPlayerControl.addbanZiFun(2);//激活一个初始板子
            GameManager.curPlayerControl.addbanZiFun(3);//激活一个初始板子
        } else if (GameManager.curLevelManager.moShi == "zuoZhanXianFeng") {
            GameManager.curPlayerControl.addbanZiFun(1);//激活一个初始板子
        ...
        this.zhiLiaoGuangTaTime2 = parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["zhiLiaoGuangTaLengQue"]) * 60;

        //无敌金钟罩计时器
        this.wuDiJinZhongZhaoTime1 = 0;
        this.wuDiJinZhongZhaoTime2 = parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["wuDiJinZhongZhaoLengQue"]) * 60;

        //金钟罩移除计时器
        this.wuDiJinZhongZhaoTime3 = 0;
        this.wuDiJinZhongZhaoTime4 = parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["wuDiJinZhongZhaoWuDiShiJian"]) * 60;
        //金钟罩是否开启
        ...
        this.daoDanHongZhaTime2 = parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["daoDanHongZhaLengQue"]) * 60;
        this.daoDanJihe = GameManager.curScene.getChildByName("daoDanJiHe");//导弹集合
        //敌人集合
        this.diRenJiHe = GameManager.curScene.getChildByName("diRenJiHe");

        //时间风暴计时器
        this.shiJianFengBaoTime1 = 0;
        this.shiJianFengBaoTime2 = parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["shiJianFengBaoLengQue"]) * 60;
        this.shiJianFengBaoTime3 = 0;
        this.shiJianFengBaoTime4 = parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["shiJianFengBaoZanTingShiJian"]) * 60;
        ...
        this.bingFengTime2 = parseInt(GameManager.jsonShuJv["guangGaoBingDongShiJian"]) * 60;
        this.bingFengKaiGuan = false;


        //玩家移速
        this.wanJiayiSu = parseInt(GameManager.jsonShuJv["wanJiaYiSu"]);
    ...
    StartGame() {
        ...
        //移除指引提示
        UIManager.gameUI.guideTip.visible = false;

        //隐藏商城和签到
        UIManager.gameUI.shangDianBut.visible = false;
        UIManager.gameUI.QianDaoBut.visible = false;
    ...
    AutoMove() {
        ...
        if (this.shanDianKaiGuan == true) {
            ...
            if (this.shanDianTime1 > this.shanDianTime2) {
                this.shanDianTime1 = 0;
                this.shanDianKaiGuan = false;
                GameManager.curScene.getChildByName("Camera").getChildByName("Camera").getChildByName("FX").getChildByName("FX_shandian_QuanPing").active = false;
            }
        ...
        if (this.bingFengKaiGuan == true) {
            ...
            UIManager.gameUI.bingDongZheZhao.alpha = UIManager.gameUI.bingDongZheZhao.alpha + 0.1;
            ...
            if (this.bingFengTime1 > this.bingFengTime2) {
                ...
                GameManager.curLevelManager.baoXueTx.active = false;//关闭全屏暴雪特效

                UIManager.gameUI.bingDongZheZhao.visible = false;//隐藏冰冻遮罩
        ...
        //作战先锋 治疗光塔等级
        if (GameManager.curLevelManager.zzxfDuiYingDeDengJi[1] > 0) {

            this.zhiLiaoGuangTaTime1 = this.zhiLiaoGuangTaTime1 + 1;
            if (this.zhiLiaoGuangTaTime1 > this.zhiLiaoGuangTaTime2) {
                this.zhiLiaoGuangTaTime1 = 0;

                var dengJi = GameManager.curLevelManager.zzxfDuiYingDeDengJi[1];
                var meiJiXueLiang = parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["zhiLiaoGuangTaZhiLiaoXueLiang"]);
        ...
        //无敌金钟罩 
        if (GameManager.curLevelManager.zzxfDuiYingDeDengJi[5] > 0) {
        ...
        //导弹轰炸 冷却处理
        if (GameManager.curLevelManager.zzxfDuiYingDeDengJi[3] > 0) {
            ...
            if (this.daoDanHongZhaTime1 > this.daoDanHongZhaTime2) {
                ...
                //投射导弹
                this.touSheDaoDan(GameManager.curLevelManager.zzxfDuiYingDeDengJi[3]);
            }
                ...
                for (var i = 0; i < this.daoDanJihe.numChildren; i++) {
                    if (this.daoDanJihe.getChildAt(i).getChildAt(0).transform.position.y <= 1) {
                        ...
                        for (var p = 0; p < this.diRenJiHe.numChildren; p++) {
                            ...
                            //距离爆炸位置近就受伤
                            if (jvLi < parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["daoDanHongZhaFanWei"])) {
                                console.log(parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["daoDanHongZhaShangHai"]));
                                this.diRenJiHe.getChildAt(p).getChildAt(0).getComponent(jiao).shouJi(parseInt(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["daoDanHongZhaShangHai"]));
                            }
                        }
        ...
        //时间风暴处理
        if (GameManager.curLevelManager.zzxfDuiYingDeDengJi[6] > 0) {
        ...
        if (this.state == "jiuYuan") {
            ...
            //相机跟随直升机
            var targetRota = new Laya.Vector3(GameManager.curScene.getChildByName("FeijI").transform.position.x, GameManager.curScene.getChildByName("FeijI").transform.position.y + 9, GameManager.curScene.getChildByName("FeijI").transform.position.z);
            ...
            //飞机飞向玩家
            var feiXiangMb = new Laya.Vector3(this.owner.transform.position.x, GameManager.curScene.getChildByName("FeijI").transform.position.y, this.owner.transform.position.z);
            var targetRota = feiXiangMb;
            var targetRotaLerp = new Laya.Vector3(0, 0, 0);
            Laya.Vector3.lerp(GameManager.curScene.getChildByName("FeijI").transform.position, targetRota, 0.005, targetRotaLerp);
            GameManager.curScene.getChildByName("FeijI").transform.position = targetRotaLerp;

            GameManager.curScene.getChildByName("FeijI").transform.lookAt(feiXiangMb, new Laya.Vector3(0, 1, 0), false);
            GameManager.curScene.getChildByName("FeijI").transform.rotationEuler = new Laya.Vector3(GameManager.curScene.getChildByName("FeijI").transform.rotationEuler.x, GameManager.curScene.getChildByName("FeijI").transform.rotationEuler.y + 180, GameManager.curScene.getChildByName("FeijI").transform.rotationEuler.z)
        }
        ...
        if (this.state == "daiJi") {
            ...
            if (this.dingWeiZhuangTai == true) {
                //相机跟随玩家
                var targetRota = GameManager.curScene.getChildByName("shiJiaoDian").transform.position;
                var targetRotaLerp = new Laya.Vector3(0, 0, 0);
                Laya.Vector3.lerp(this.xiangJiFa.transform.position, targetRota, 0.06, targetRotaLerp);
                this.xiangJiFa.transform.position = targetRotaLerp;
            } else {
        ...
        if (this.state == "yiDong") {
            ...
            if (jvLi <= 0.3) {
                ...
                if (this.menObj.transform.position.y <= -10) {
                    ...
                    //加载下一关敌人
                    GameManager.curLevelManager.xiaYiGuanJiaZai();
        ...
        //准备战斗
            ...
            var targetRota = GameManager.curScene.getChildByName("LV").getChildByName("luojiwuti").getChildByName("WinObj").getChildByName("zhanDouDian").transform.position;
            var targetRotaLerp = new Laya.Vector3(0, 0, 0);
            ...
            //相机跟随玩家
            GameManager.cameraTrans.position = this.moveTrans.position;
            GameManager.cameraTrans.rotationEuler = this.moveTrans.rotationEuler;

            var jvLi = Laya.Vector3.distance(this.owner.transform.position, GameManager.curScene.getChildByName("LV").getChildByName("luojiwuti").getChildByName("WinObj").getChildByName("zhanDouDian").transform.position)
            ...
            if (GameManager.curScene.getChildByName("xiaoDaoJiHe").getChildAt(this.daoYuSuoYin).getChildByName("QiZi").getComponent(Laya.Animator).getCurrentAnimatorPlayState()._normalizedTime > 0.95) {
                GameManager.curScene.getChildByName("xiaoDaoJiHe").getChildAt(this.daoYuSuoYin).getChildByName("QiZi").getComponent(Laya.Animator).play("2", 0, 0);
            }
                    ...
                    if (i == 0) {
                        ...
                        if (jvLi < 1) {
                            ...
                            for (var p = 0; p < shuLiang; p++) {
                                var tx = Laya.Sprite3D.instantiate(GameManager.models.getChildByName("FX_shengji"), GameManager.curScene);
                            ...
                            for (var p = 0; p < shuLiang; p++) {
                                ...
                                if (GameManager.RandomInt(1, 2) == 1) {
                                    ziFu = "chongFeng_Bing";
                                } else {
                                    ziFu = "buQiang_Bing";
                                }

                                var sqBing = Laya.Sprite3D.instantiate(GameManager.models.getChildByName(ziFu), this.fuLuJihe);
                                sqBing.transform.position = weizhiShuZu[p];
                                sqBing.getChildAt(1).skinnedMeshRenderer.material = GameManager.models.getChildByName("piFuLan").meshRenderer.material;
                                sqBing.getChildAt(2).skinnedMeshRenderer.material = GameManager.models.getChildByName("piFuLan").meshRenderer.material;
            ...
            if (this.benxiangWanJia == true) {
                for (var i = 0; i < this.fuLuJihe.numChildren; i++) {
                    this.fuLuJihe.getChildAt(i).transform.lookAt(GameManager.curPlayerControl.owner.transform.position, new Laya.Vector3(0, 1, 0), false);
                    this.fuLuJihe.getChildAt(i).transform.rotationEuler = new Laya.Vector3(this.fuLuJihe.getChildAt(i).transform.rotationEuler.x, this.fuLuJihe.getChildAt(i).transform.rotationEuler.y + 180, this.fuLuJihe.getChildAt(i).transform.rotationEuler.z)
                    ...
                    var jvLi = Laya.Vector3.distance(GameManager.curPlayerControl.owner.transform.position, this.fuLuJihe.getChildAt(i).transform.position);
                    if (jvLi < 3) {

                        if (this.fuLuJihe.getChildAt(i).active == true) {

                            GameManager.PlaySound("heCheng");
                            ...
                            if (i == 0) {
                                ...
                                //岛屿碰撞由内向外缩放 放置卡住玩家
                                GameManager.curScene.getChildByName("xiaoDaoJiHe").getChildAt(this.daoYuSuoYin).getChildByName("pengZhuangJiHe").getComponent(Laya.Animator).play("daoPengZhuangKs", 0, 0);
                            }
                        }
    ...
    jiuYuanFun() {
        ...
        UIManager.gameUI.yoUXiUI.visible = false;
        UIManager.gameUI.jiNnegZuUIFa.visible = false;

        UIManager.ShowGameTipUI("救援直升机已抵达,准备登机");
        //救援音效
        GameManager.PlaySound("zhiShengJi");
        GameManager.PlaySound("duiJiangJi");
    }
    ...
    kaiMenFun() {
        ...
        //获取要操作的们
        this.menObj = GameManager.curScene.getChildByName("menJiHe").getChildAt(GameManager.curLevelManager.dangQianGuanQia);

        GameManager.PlaySound("shiMenKai");
    ...
    fuLuBiaoYanFun(daoYuSuoYin) {
        ...
        //俘虏集合
        this.fuLuJihe = GameManager.curScene.getChildByName("xiaoDaoJiHe").getChildAt(daoYuSuoYin).getChildByName("fuLuJiHe");

        //笼子集合
        this.longZiJiHe = GameManager.curScene.getChildByName("xiaoDaoJiHe").getChildAt(daoYuSuoYin).getChildByName("longZiJiHe");
        //箱子
        this.xiangZi = GameManager.curScene.getChildByName("xiaoDaoJiHe").getChildAt(daoYuSuoYin).getChildByName("xiangZi");
        ...
        //旗子降落动画
        GameManager.curScene.getChildByName("xiaoDaoJiHe").getChildAt(daoYuSuoYin).getChildByName("QiZi").getComponent(Laya.Animator).play("QiZi", 0, 0);
    ...
    addbanZiFun(leiXing) {
        ...
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            if (this.banZiJiHe.getChildAt(i).transform.position.y < -10) {
                ...
                this.banZiJiHe.getChildAt(i).getComponent(CwControl).jiHuoFun(leiXing);
        ...
        if (leiXing == 0) {
            ...
            for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
                if (this.banZiJiHe.getChildAt(i).transform.position.y > -10) {
                    if (this.banZiJiHe.getChildAt(i).getComponent(CwControl).leiXing == 0) {
            ...
            if (shu.length >= 4) {
                ...
                if (this.state == "shiBai") {
                    ...
                    //弹出引导
                    if (PlayerData.GetUserData("fuBanHeShouQiangBingYd") == false) {
                        UIManager.gameUI.yindaoTanChuang(UIManager.gameUI.banZiHeCheng);
                    }


                    GameManager.PlaySound("heCheng")
                    for (var i = 0; i < shu.length; i++) {
                        shu[i].getComponent(CwControl).gouDaiTouFun();
                    }
                    this.addbanZiFun(1);
                }
            ...
            for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
                if (this.banZiJiHe.getChildAt(i).transform.position.y > -10) {
                    if (this.banZiJiHe.getChildAt(i).getComponent(CwControl).leiXing == 1) {
                        shu.push(this.banZiJiHe.getChildAt(i));
                    }
                }
            }
            ...
            if (shu.length >= 3) {
                ...
                if (PlayerData.GetUserData("xiangTongShiBingHeChengYd") == false) {
                    ...
                    UIManager.gameUI.yindaoTanChuang(UIManager.gameUI.shiBingHeCheng);
                }


                UIManager.gameUI.shouQIangdShengJiUI.visible = true;//手枪兵进阶弹窗
                this.jinJieJiHe = shu;//三个 一级兵集合

                //暂停
                Laya.timer.frameOnce(8, this, function () {

                    if (UIManager.gameUI.shouQIangdShengJiUI.visible == false && UIManager.gameUI.chongFengQiangShengJiUI.visible == false && UIManager.gameUI.buQiangShengJiUI.visible == false || this.state == "shengLi" || this.state == "shiBai") {
            ...
            for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
                if (this.banZiJiHe.getChildAt(i).transform.position.y > -10) {
                    if (this.banZiJiHe.getChildAt(i).getComponent(CwControl).leiXing == 2) {
                        shu.push(this.banZiJiHe.getChildAt(i));
                    }
                }
            }
            ...
            if (shu.length >= 3) {
                ...
                UIManager.gameUI.chongFengQiangShengJiUI.visible = true;//步枪或者冲锋枪 进阶弹窗
                this.jinJieJiHe = shu;
            ...
            for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
                if (this.banZiJiHe.getChildAt(i).transform.position.y > -10) {
                    if (this.banZiJiHe.getChildAt(i).getComponent(CwControl).leiXing == 3) {
                        shu.push(this.banZiJiHe.getChildAt(i));
                    }
                }
            }
            ...
            if (shu.length >= 3) {
                UIManager.gameUI.buQiangShengJiUI.visible = true;//步枪 进阶弹窗
                this.jinJieJiHe = shu;
    ...
    chongFengQiangHc() {
        ...
        for (var i = 0; i < this.jinJieJiHe.length; i++) {
            this.jinJieJiHe[i].getComponent(CwControl).gouDaiTouFun();
        }
        this.addbanZiFun(2);
    ...
    fanHuiRenShu() {
        ...
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            ...
            if (geShu.parent.getComponent(CwControl).state == "gouDai" || geShu.parent.getComponent(CwControl).state == "gouDaiTou") {
    ...
    shengShenZhuFu() {
        ...
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            ...
            if (this.banZiJiHe.getChildAt(i).transform.position.y < -10) {
                ...
                var chengDeShu = parseFloat(GameManager.jsonShuJv["zuoZhanXianFengMoShiShuJv"]["shengShenZhuFuZengJiaXueLiang"]);

                var yaoZengJiaDe = this.banZiJiHe.getChildAt(i).getComponent(CwControl).xueLiang * chengDeShu;

                this.banZiJiHe.getChildAt(i).getComponent(CwControl).zhiLiao(yaoZengJiaDe);
    ...
    quanTizhiLiao(shuLiang) {
        ...
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            ...
            if (this.banZiJiHe.getChildAt(i).transform.position.y < -10) {
                ...
                // console.log(shuLiang);
                this.banZiJiHe.getChildAt(i).getComponent(CwControl).zhiLiao(shuLiang);
    ...
    quanTiHuDun() {
        ...
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            ...
            if (this.banZiJiHe.getChildAt(i).transform.position.y < -10) {
                ...
                this.banZiJiHe.getChildAt(i).getComponent(CwControl).huDun();
    ...
    quanTiHuDunYiChu() {
        ...
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            ...
            if (this.banZiJiHe.getChildAt(i).transform.position.y < -10) {
                ...
                this.banZiJiHe.getChildAt(i).getComponent(CwControl).yiChuHuDun();
    ...
    teShuJiaGu() {
        ...
        for (var i = 0; i < this.banZiJiHe.numChildren; i++) {
            if (this.banZiJiHe.getChildAt(i).transform.position.y < -10) {
                ...
                this.banZiJiHe.getChildAt(i).getComponent(CwControl).xueLiang = this.banZiJiHe.getChildAt(i).getComponent(CwControl).xueLiang + tiShengDeXueLiang;
                this.banZiJiHe.getChildAt(i).getComponent(CwControl).dangQianXueLiang = this.banZiJiHe.getChildAt(i).getComponent(CwControl).dangQianXueLiang + tiShengDeXueLiang

                this.banZiJiHe.getChildAt(i).getComponent(CwControl).shuaXinXueLiangXianShi();//刷新血量显示
            }
        }
    ...
    touSheDaoDan(num) {
        ...
        for (var i = 0; i < num; i++) {
            ...
            GameManager.PlaySound("daoDanSou");

            var suiJiShu = GameManager.RandomInt(0, this.diRenJiHe.numChildren - 1);

            var jvLi = Laya.Vector3.distance(this.diRenJiHe.getChildAt(i).transform.position, GameManager.curPlayerControl.owner.transform.position);
            if (jvLi < 20) {
                var dr = Laya.Sprite3D.instantiate(GameManager.models.getChildByName("daoDanFa"), GameManager.curScene.getChildByName("daoDanJiHe"));
    ...
    shiJianFengBaoChuFa() {
        ...
        GameManager.PlaySound("baoXue");
        UIManager.gameUI.bingDongZheZhao.visible = true;//显示冰冻遮罩
        UIManager.gameUI.bingDongZheZhao.alpha = 0.1;

        GameManager.curLevelManager.baoXueTx.active = true;//显示全屏暴雪特效
    ...
    sheXianJiance() {
        ...
        //进行射线检测,检测第一个碰撞物体
        GameManager.curScene.physicsSimulation.raycastAllFromTo(this.sheXianQiDian.transform.position, this.sheXianZhongDian.transform.position, this.hitResults);
    ...
    yinBaoFuJinYouTong() {
        ...
        this.fuJinYouTong.getComponent(youTong).shouJi(1000);
    ...
    shanDianFengBao() {
        ...
        GameManager.PlaySound("shanDian");

        this.shanDianKaiGuan = true;
        GameManager.curScene.getChildByName("Camera").getChildByName("Camera").getChildByName("FX").getChildByName("FX_shandian_QuanPing").active = true;
        ...
        for (var i = 0; i < this.diRenJiHe.numChildren; i++) {
            ...
            for (var p = 0; p < this.diRenJiHe.getChildAt(i).numChildren; p++) {
                ...
                //不秒杀 直接省丝血
                if (parseInt(GameManager.jsonShuJv["guangGaoLeiDianShangHai"]) > this.diRenJiHe.getChildAt(i).getChildAt(p).getComponent(jiao).dangQianXueLiang) {
                    this.diRenJiHe.getChildAt(i).getChildAt(p).getComponent(jiao).shouJi(this.diRenJiHe.getChildAt(i).getChildAt(p).getComponent(jiao).dangQianXueLiang - 1);

                } else {
                    this.diRenJiHe.getChildAt(i).getChildAt(p).getComponent(jiao).shouJi(parseInt(GameManager.jsonShuJv["guangGaoLeiDianShangHai"]));
    ...
    onTriggerEnter(other) {
        ...
        if (oname.indexOf("barrel01_A") != -1) {
            UIManager.gameUI.yinBaoYouTongBut.visible = true;
    ...
    onTriggerExit(other) {
        ...
        //离开油桶
        if (oname.indexOf("barrel01_A") != -1) {
            UIManager.gameUI.yinBaoYouTongBut.visible = false;
    ...
    erDShuaXin(scenePos) {
        ...
        GameManager.curScene.getChildByName("CameraRoot").getChildByName("AnimatorRoot").getChildByName("TranslateNode").getChildByName("EulerY").getChildByName("EulerX").getChildByName("Main Camera").worldToViewportPoint(scenePos, screenPos4);
    ...
    duiZhangShengWuQi() {
        ...
        this.duiZhangObj.getComponent(CwControl).wt.active = false;
        GameManager.curScene.addChild(this.duiZhangObj.getComponent(CwControl).wt)

        if (this.duiZhangObj.getComponent(CwControl).leiXing == 1) {
            this.duiZhangObj.getComponent(CwControl).leiXing = 2;
        } else if (this.duiZhangObj.getComponent(CwControl).leiXing == 2) {
            this.duiZhangObj.getComponent(CwControl).leiXing = 3;
        } else if (this.duiZhangObj.getComponent(CwControl).leiXing == 3) {
            this.duiZhangObj.getComponent(CwControl).leiXing = 6;
        } else if (this.duiZhangObj.getComponent(CwControl).leiXing == 6) {
            this.duiZhangObj.getComponent(CwControl).leiXing = 4;
        } else if (this.duiZhangObj.getComponent(CwControl).leiXing == 4) {
            this.duiZhangObj.getComponent(CwControl).leiXing = 5;
        }


        this.duiZhangObj.getComponent(CwControl).jiHuoFun(this.duiZhangObj.getComponent(CwControl).leiXing);
    ...
    shuaXinXuanWo() {
        ...
        for (var i = 0; i < GameManager.curLevelManager.zzxfDuiYingDeDengJi[4]; i++) {
            GameManager.curLevelManager.xuanWoFa.getChildAt(i).active = true;
        }
    ...
    shiBaiChuLiFun() {
        ...
        Laya.timer.frameOnce(120, this, function () {
            ...
            GameManager.PlaySound("shiBai");
            //过两秒弹出宝箱页面
            UIManager.ShowUIPanel("Fail", false);
        });
    ...
    suanJinDu() {
        ...
        UIManager.gameUI.jinDu.width = 200 * GameManager.curScene.getChildByName("PlayerStart").getComponent(Laya.Animator).getCurrentAnimatorPlayState()._normalizedTime;
